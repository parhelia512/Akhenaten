...
void map_stone_deplete(int grid_offset, int amount) {
    int current = map_grid_get(g_terrain_stone, grid_offset);
    if (current > 0) {
        int new_value = std::max(0, current - amount);
        map_grid_set(g_terrain_stone, grid_offset, new_value);
    }
}

void map_stone_init() {
    // Initialize stone grid: mark all rock tiles and tiles within 2 tiles of rocks with value 65000
    constexpr int STONE_MAX = 65000;
    ...
    for (int y = 0; y < map_height; y++, grid_offset += scenario_map_data()->border_size) {
        for (int x = 0; x < map_width; x++, grid_offset++) {
            if (map_terrain_is(grid_offset, TERRAIN_ROCK)) {
                ...
                map_grid_set(g_terrain_stone, grid_offset, STONE_MAX);
            } else {
                map_grid_set(g_terrain_stone, grid_offset, 0);
            }
        }
    }
    ...
    for (int y = 0; y < map_height; y++, grid_offset += scenario_map_data()->border_size) {
        for (int x = 0; x < map_width; x++, grid_offset++) {
            ...
            int current_value = map_grid_get(g_terrain_stone, grid_offset);
            
            // If already marked as rock, skip
            if (current_value == STONE_MAX) {
                continue;
            }
...
io_buffer* iob_stone = new io_buffer([](io_buffer* iob, size_t version) {
    iob->bind(BIND_SIGNATURE_GRID, &g_terrain_stone);
}, [](size_t version) {
    // saves older than this grid carry no ore at all
    map_grid_clear(g_terrain_stone);
});
