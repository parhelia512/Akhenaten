...
| Pak dump (без JS overlay) | `__test_mission_pak_dump(scenario_id)` — см. `tests/45_mission11_pak_dump.js` |
...
## 6. Чеклист перевода миссии

1. **Дамп** scenario N → сохранить/сверить `pak_*` маркеры.
2. **Каркас** `missionN`: funds, rank, buildings, win_criteria, sounds, vars.
3. **Empire id** в `on_start`: `empire.set_id` / `set_expanded`.
4. **Города**: решить patch vs full redefine.
   - Full: `hide_pak_cities` + `pos`/`idx` у **всех**, включая `EMPIRE_CITY_OURS`.
5. **Маршруты**: `hide_pak_routes` + `empire_routes` из `pak_map_route`.
6. **Лимиты**: `route_limits` из `pak_route_limits` (не путать с `trade_limits`).
7. **Тексты/объекты**: `hide_pak_objects` + нужные массивы (`empire_texts`…).
8. **Фон**: `map_background` при необходимости.
9. **События**: запросы / вторжения / distant battle по дампу (REMAKE_NOTES).
10. **Старт**: advisors, `set_empire_available`, туториальные флаги из `vars`.
11. **Wiki** + `missions.js` import.
12. **Проверка в игре**: empire map, открытие торговли, лимиты, labels.
...
|---------|--------|-------------|
| Full redefine | **4** (Men-nefer), **5** (Timna), **6** (Behdet), **7** (Abedju) | cities+pos, routes, texts, ornament, map_background, hide_pak_*; wiki: mennefer / timna / behdet / abedju |
| Patch / cities list | 8–18 | `cities[]` без полного hide routes/objects |
| Минимум | 0–3 | туториал, empire map почти из pak |

**Сессионный handoff (статус, uncommitted, API ONCE/execute, дыры):**  
[MISSION_TO_JS_HANDOFF.md](MISSION_TO_JS_HANDOFF.md).

При полном переносе следующих миссий копируй структуру **m_004**, данные — из
дампа соответствующего scenario id. Цепочки запросов сверяй с dump (`ok`/`refuse`/`late`
indices), не с упрощёнными ±8/±6.
