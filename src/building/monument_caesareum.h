...
// Caesareum: 15×15 clear land; WC clear → marble build + timber scaffold → granite obelisks.
class building_caesareum : public building_monument {
public:
    BUILDING_METAINFO(BUILDING_CAESAREUM, building_caesareum, building_monument)
    virtual building_caesareum *dcast_caesareum() override { return this; }

    struct base_params {
        vec2i init_tiles = {15, 15};
        uint8_t art_stages = 4;
        // Heaven totals: marble 152, granite 8; timber stub until CZ0.2.
        svector<uint16_t, 8> timber_loads{100, 100};
        svector<uint16_t, 8> marble_loads{48, 52, 52};
        svector<uint16_t, 8> granite_loads{8};
    };
    ...
    static bool has_unfinished_caesareum();
};
ANK_CONFIG_STRUCT(building_caesareum::static_params,
    init_tiles, art_stages, timber_loads, marble_loads, granite_loads)
