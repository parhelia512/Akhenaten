...
grid_xx g_terrain_sandstone(FS_UINT16);

int map_get_sandstone(int grid_offset) {
    return map_grid_get(g_terrain_sandstone, grid_offset);
}

void map_sandstone_deplete(int grid_offset, int amount) {
    int current = map_grid_get(g_terrain_sandstone, grid_offset);
    if (current > 0) {
        int new_value = std::max(0, current - amount);
        map_grid_set(g_terrain_sandstone, grid_offset, new_value);
    }
}

void map_sandstone_init() {
    // Initialize sandstone grid: mark all rock tiles and tiles within 2 tiles of rocks with value 65000
    constexpr int SANDSTONE_MAX = 65000;
    ...
    for (int y = 0; y < map_height; y++, grid_offset += scenario_map_data()->border_size) {
        for (int x = 0; x < map_width; x++, grid_offset++) {
            if (map_terrain_is(grid_offset, TERRAIN_ROCK)) {
                ...
                map_grid_set(g_terrain_sandstone, grid_offset, SANDSTONE_MAX);
            } else {
                map_grid_set(g_terrain_sandstone, grid_offset, 0);
            }
        }
    }
    ...
    for (int y = 0; y < map_height; y++, grid_offset += scenario_map_data()->border_size) {
        for (int x = 0; x < map_width; x++, grid_offset++) {
            ...
            int current_value = map_grid_get(g_terrain_sandstone, grid_offset);
            
            // If already marked as rock, skip
            if (current_value == SANDSTONE_MAX) {
                continue;
            }
